...
function editorHtml(c){
 ...
 if(editing.kind==='add'){label='';value='';canRename=true}
  ...
  const labels={profile:'人物像・背景',world:'世界',start:'開始地点',bonds:'絆'};
  label=labels[editing.id]||editing.id;value=standardValue(c,editing.id)
 }
 return '<div class="about-editor"><div class="about-editor-title">'+(editing.kind==='add'?'項目を追加':'編集')+'</div>'+(canRename?'<label>項目名<input id="aboutFieldLabel" maxlength="40" value="'+esc(label)+'" placeholder="例：故郷、家族、アイアンランドの真実"></label>':'<div class="about-fixed-label">'+esc(label)+'</div>')+'<label>内容<textarea id="aboutFieldValue" rows="4" placeholder="後から決めても大丈夫です。">'+esc(value)+'</textarea></label><div class="about-editor-actions"><button id="aboutCancelBtn" type="button">キャンセル</button><button id="aboutSaveBtn" class="primary" type="button">保存</button></div></div>'
}
function render(){
 const save=read(),c=normalize(save),box=$('aboutContent');if(!c||!box)return;
 const standard=[['人物像・背景',standardValue(c,'profile'),'profile'],['世界',standardValue(c,'world'),'world'],['開始地点',standardValue(c,'start'),'start'],['絆',standardValue(c,'bonds'),'bonds']];
 let html='<h2>'+esc(c.name||save.name||'主人公')+'</h2>';
 html+=standard.map(x=>row(x[0],x[1],'standard',x[2])).join('');
 html+=c.aboutFields.map(x=>row(x.label||'名称未設定',x.value||'','custom',x.id,true)).join('');
