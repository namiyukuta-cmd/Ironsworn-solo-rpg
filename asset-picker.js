...
let ch=(pack&&pack.id===charId&&pack.character)?pack.character:(charId&&store[charId]?JSON.parse(JSON.stringify(store[charId])):null);
...
function sameAsset(x,a){return !!x&&(x.assetId===a.id||x.id===a.id||x.name===a.name)}
...
function renderList(){
  ...
  arr.forEach(a=>{
    ...
    c.innerHTML=`<div class="card-top"><div class="name">${a.name}${a.userCustom?' <span class="user-custom-tag">自作</span>':''}</div><div class="type">${a.type}</div></div><div class="summary"><b>ざっくり：</b>${easy(a.summary)}</div>${a.requirement?`<div class="req"><b>取得条件：</b>${easy(a.requirement)}</div>`:""}<div class="what-title">このアセットでできること</div><div class="abilities">${(a.abilities||[]).map((x,i)=>`<div class="ability"><div class="ability-name"><span class="num">${i+1}</span><b>${x.label}</b>${i===0&&!a.chooseStartingAbility?'<span class="start-tag">最初から</span>':''}</div><div class="effect"><span class="effect-label">使うと</span><span>${plainEffect(x.description)}</span></div></div>`).join("")}</div><button type="button" class="choose" ${own?"disabled":""}>${own?"取得済み":current?"このままにする":replaceIndex!==null?"これに変更":cost()?"3 XPで取得":"選ぶ"}</button>`;
    const chooseBtn=c.querySelector(".choose");if(!own)chooseBtn.onclick=()=>begin(a);el.list.appendChild(c);
  })
}
function begin(a){
  if(owned(a))return;
  ...
  if(isCurrent(a)){
    const cur=ch.assets[replaceIndex],marked=(cur.abilities||[]).findIndex(x=>Array.isArray(x)?x[1]:x.active);pendingAbility=marked>=0?marked:0;
  }
  ...
  if(a.chooseStartingAbility)(a.abilities||[]).forEach((x,i)=>{
    const b=document.createElement("button");b.type="button";b.className="start-ability"+(i===pendingAbility?" selected":"");b.innerHTML=`<strong>${x.label}</strong><span>${plainEffect(x.description)}</span>`;
    b.onclick=()=>{pendingAbility=i;el.startAbilities.querySelectorAll(".start-ability").forEach((z,j)=>z.classList.toggle("selected",j===i))};el.startAbilities.appendChild(b);
  });
...
function confirmAssetSelection(){
  ...
  const obj={
    assetId:pending.id,name:pending.name,type:pending.type,summary:pending.summary||"",
    abilities:(pending.abilities||[]).map((x,i)=>[x.label,i===pendingAbility,x.description||""]),
    source:pending.userCustom?"user-custom":"asset-library",userCustom:!!pending.userCustom
  };
...
function loadUserCustoms(){try{return JSON.parse(localStorage.getItem(CUSTOM_KEY)||"[]")||[]}catch(e){return[]}}
function saveUserCustoms(items){localStorage.setItem(CUSTOM_KEY,JSON.stringify(items))}
function openCustom(){
  el.customError.textContent="";[el.customName,el.customSummary,el.customA1,el.customD1,el.customA2,el.customD2,el.customA3,el.customD3].forEach(x=>x.value="");el.customType.value="カスタム";
  el.customModal.classList.add("open");el.customModal.setAttribute("aria-hidden","false");
}
...
function createCustom(){
  ...
  const a={id:"user-custom-"+Date.now(),name,type:el.customType.value,summary:el.customSummary.value.trim()||"自作のカスタムアセット。",abilities,chooseStartingAbility:abilities.length>1,requirement:null,custom:true,userCustom:true};
  const customs=loadUserCustoms();customs.push(a);saveUserCustoms(customs);data.push(a);closeCustom();renderList();begin(a);
}

el.backBtn.onclick=()=>location.href="index.html#game";
el.search.oninput=e=>{query=e.target.value;renderList()};
el.cancelBtn.onclick=closeModal;el.confirmBtn.onclick=confirmAssetSelection;el.modal.onclick=e=>{if(e.target===el.modal)closeModal()};
...
Promise.all(DATA_FILES.map(f=>fetch(`${f}?v=asset5`,{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error(f);return r.json()}))).then(parts=>{
  data=parts.flat().concat(loadUserCustoms());status();renderFilters();renderList();
}).catch(err=>{console.error(err);el.list.innerHTML='<div class="empty">アセットデータを読み込めませんでした。</div>'});
})();